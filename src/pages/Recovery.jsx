...
export default function Recovery() {
  ...
  return (
    <div style={{ backgroundColor: '#FFF8EF', minHeight: '100vh' }}>
      ...
      {/* Hero */}
      <section style={{ background: 'linear-gradient(135deg, #F07070, #F59595)', padding: '56px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
          ...
          <h1 style={{ color: '#fff', fontSize: 'clamp(28px, 5vw, 44px)', fontWeight: '700', margin: '0 0 16px', letterSpacing: '-0.5px' }}>
      ...
      <section style={{ padding: '56px 24px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <h2 style={{ fontSize: '28px', fontWeight: '700', color: '#1B6B6B', margin: '0 0 8px', textAlign: 'center' }}>주차별 회복 프로그램</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '24px' }}>
            {weeklyProgram.map(({ week, title, focus, color, bg, items }) => (
              <div key={week} style={{ backgroundColor: '#fff', borderRadius: '16px', overflow: 'hidden', border: '1px solid #E8D5BC' }}>
                <div style={{ backgroundColor: bg, padding: '20px 24px', borderBottom: `3px solid ${color}` }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', color, textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>{week}</div>
                  <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#1a202c', margin: '0 0 4px' }}>{title}</h3>
                  <p style={{ fontSize: '13px', color: '#6B7280', margin: 0 }}>{focus}</p>
                </div>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          padding: '10px 0', textAlign: 'left', borderBottom: idx < items.length - 1 ? '1px solid #F0E6D3' : 'none',
                        }}
                      >
                        {done
                        ...
                        <span style={{ fontSize: '14px', color: done ? '#9CA3AF' : '#374151', textDecoration: done ? 'line-through' : 'none', lineHeight: '1.5' }}>
                          {text}
                        </span>
                      </button>
                    )
      ...
      <section style={{ padding: '0 24px 64px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <h2 style={{ fontSize: '28px', fontWeight: '700', color: '#1B6B6B', margin: '0 0 8px', textAlign: 'center' }}>하루 루틴 가이드</h2>
          <p style={{ color: '#6B7280', fontSize: '15px', textAlign: 'center', marginBottom: '40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px' }}>
            {dailyTips.map(({ title, tips }) => (
              <div key={title} style={{
                ...
                <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#1B6B6B', margin: '0 0 16px' }}>{title}</h3>
                <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {tips.map(tip => (
                    <li key={tip} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '14px', color: '#374151', lineHeight: '1.5' }}>
                      <span style={{ color: '#D4A422', fontWeight: '700', flexShrink: 0, marginTop: '1px' }}>✓</span>
                      {tip}
                    </li>
                  ))}
                </ul>
      ...
      {/* Notice */}
      <div style={{ maxWidth: '1100px', margin: '0 24px 48px', padding: '20px 24px', backgroundColor: '#FDF6E3', border: '1px solid #E8BC3A', borderRadius: '12px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
        <span style={{ fontSize: '20px' }}>⚠️</span>
        <p style={{ margin: 0, fontSize: '14px', color: '#6B4C00', lineHeight: '1.6' }}>
