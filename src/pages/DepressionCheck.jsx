...
export default function DepressionCheck() {
  ...
  if (submitted) {
    return (
      <div style={{ backgroundColor: '#FFF8EF', minHeight: '100vh', padding: '48px 24px' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
          <div style={{
            ...
            <div style={{ fontSize: '13px', color: '#9CA3AF', marginBottom: '8px' }}>검사 결과</div>
            <h2 style={{ fontSize: '28px', fontWeight: '700', color: result.color, margin: '0 0 8px' }}>
              {result.level}
            </h2>
            <div style={{ fontSize: '32px', fontWeight: '700', color: '#1a202c', margin: '0 0 24px' }}>
              {score}점 <span style={{ fontSize: '16px', color: '#9CA3AF', fontWeight: '400' }}>/ 30점</span>
            </div>

            {/* Score bar */}
            <div style={{ backgroundColor: '#F3F4F6', borderRadius: '8px', height: '12px', marginBottom: '8px', position: 'relative', overflow: 'hidden' }}>
              <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#9CA3AF', marginBottom: '28px' }}>
              <span>0 (정상)</span><span>9</span><span>13</span><span>30 (주의)</span>
            </div>

            <p style={{ fontSize: '15px', color: '#374151', lineHeight: '1.7', margin: '0 0 32px', padding: '16px', backgroundColor: result.bg, borderRadius: '12px' }}>
              {result.desc}
            </p>

            {answers[10] >= 2 && (
              <div style={{ backgroundColor: '#FEF0F0', border: '1px solid #F59595', borderRadius: '12px', padding: '16px', marginBottom: '24px', fontSize: '14px', color: '#7F1D1D', lineHeight: '1.6' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: '10px', border: '2px solid #E8D5BC', background: '#fff',
              ...
              <a
                ...
                style={{
                  display: 'flex', alignItems: 'center', gap: '6px', padding: '12px 24px',
                  borderRadius: '10px', border: 'none', backgroundColor: '#1B6B6B',
  ...
  return (
    <div style={{ backgroundColor: '#FFF8EF', minHeight: '100vh' }}>
      ...
      {/* Hero */}
      <section style={{ background: 'linear-gradient(135deg, #D4A422, #E8BC3A)', padding: '48px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
        ...
        <div style={{ maxWidth: '680px', margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: '32px' }}>
            ...
            <div style={{ backgroundColor: '#E5E7EB', borderRadius: '8px', height: '8px', overflow: 'hidden' }}>
              <div style={{ height: '100%', backgroundColor: '#D4A422', borderRadius: '8px', width: `${((current + 1) / questions.length) * 100}%`, transition: 'width 0.3s' }} />
            </div>
          </div>

          {/* Question Card */}
          <div style={{ backgroundColor: '#fff', borderRadius: '20px', padding: '36px', border: '1px solid #E8D5BC', marginBottom: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
            {q.warning && (
              <div style={{ backgroundColor: '#FEF0F0', border: '1px solid #F59595', borderRadius: '8px', padding: '10px 14px', marginBottom: '20px', fontSize: '13px', color: '#7F1D1D' }}>
                이 질문은 민감한 내용을 포함합니다. 편안하게 솔직하게 답변해주세요.
              </div>
            )}
            <p style={{ fontSize: '18px', fontWeight: '600', color: '#1a202c', lineHeight: '1.5', margin: '0 0 28px' }}>
              {current + 1}. {q.text}
            </p>
              ...
              {q.options.map((opt, i) => (
                <button
                  ...
                  style={{
                    ...
                    border: `2px solid ${answers[q.id] === i ? '#D4A422' : '#E5E7EB'}`,
                    backgroundColor: answers[q.id] === i ? '#FDF6E3' : '#fff',
                    cursor: 'pointer', fontSize: '15px', color: '#374151',
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'space-between' }}>
            <button
              ...
              disabled={current === 0}
              style={{
                padding: '12px 24px', borderRadius: '10px', border: '2px solid #E8D5BC',
                background: '#fff', cursor: current === 0 ? 'not-allowed' : 'pointer',
                fontSize: '14px', fontWeight: '600', color: '#374151', opacity: current === 0 ? 0.4 : 1,
              }}
            >
              이전
            </button>
            {current < questions.length - 1 ? (
              <button
                ...
                style={{
                  ...
                  backgroundColor: '#1B6B6B', cursor: 'pointer',
              ...
              <button
                ...
                style={{
                  padding: '12px 24px', borderRadius: '10px', border: 'none',
                  backgroundColor: allAnswered ? '#D4A422' : '#D1D5DB',
                  cursor: allAnswered ? 'pointer' : 'not-allowed',
                  fontSize: '14px', fontWeight: '700', color: allAnswered ? '#1B6B6B' : '#9CA3AF',
                  display: 'flex', alignItems: 'center', gap: '6px',
                }}
              >
